...
"""
Baseline GA Wrapper for Macro→Micro Variant (Normalized Output Interface)

This module exposes a standardized interface for executing the Macro→Micro
Genetic Algorithm across heterogeneous environments. It enforces guard-rails
for crossover probabilities, normalizes outputs to the fine-tuning schema,
and provides single-environment and batch-style entry points.

Responsibilities
---------------
- Execute the Macro→Micro GA using base-level and candidate-specific
  hyperparameters.
- Validate crossover probabilities and optionally allow mutation-only runs.
- Control runtime verbosity in a unified manner.
- Return portable, JSON-serializable payloads with minimal metadata.

Public API
----------
- run_baseline_w1(graph_links, price_matrix, agent_info_dict, hyperparams) -> Dict
- run_baseline_w2(env_dict, base_hyperparams, candidate_hyperparams, env_id=None) -> Dict
...
def _guard_crossover(p_macro: float, p_micro: float,
                     allow_mutation_only: bool, strict: bool) -> None:
    """
    ...
    - If both p_macro and p_micro are zero and mutation-only is not allowed,
      either raise ValueError (strict=True) or emit a warning (strict=False).
    """
    pm = float(p_macro)
    pn = float(p_micro)
    if pm <= 0.0 and pn <= 0.0 and not allow_mutation_only:
        ...
        if strict:
            ...
            print(f"[baseline.wrapper] WARNING: {msg}")


def _coerce_hparams(hp: Dict[str, Any]) -> Dict[str, Any]:
    """
    Coerce hyperparameter types to robust primitives.
    """
    out = dict(hp)
    ...
    # Floats
    for k in ("elite_fraction", "lambda_in", "lambda_out", "p_macro", "p_micro",
...
def run_baseline_w1(
    ...
    """
    Execute the Macro→Micro GA on a single environment.
    ...
    # Crossover probabilities and guard-rails.
    p_macro = float(hp.get("p_macro", 0.0))
    p_micro = float(hp.get("p_micro", 0.0))
    allow_mutation_only = bool(hp.get("allow_mutation_only", True))
    strict = bool(hp.get("strict", True))
    _guard_crossover(p_macro, p_micro, allow_mutation_only, strict)
    ...
    notes["wrapper"] = "baseline_macro_micro"
    notes["mutation_only"] = (p_macro == 0.0 and p_micro == 0.0)
    notes["allow_mutation_only"] = allow_mutation_only
    meta["algo"] = ALGO_NAME
...
def run_baseline_w2(
    ...
    if int(hp_merged.get("verbosity", 1)) > 0:
        print(
            f"Starting Baseline Macro→Micro GA | {name} | "
            f"gens={hp_merged.get('generations')} | pop={hp_merged.get('popsize')} | "
            f"parents_rate={hp_merged.get('parents_rate', None)} | "
            f"p_macro={hp_merged.get('p_macro', 0.0)} | p_micro={hp_merged.get('p_micro', 0.0)}"
        )

    t0 = time.time()
    res = run_baseline_w1(graph, prices, agents, hp_merged)
