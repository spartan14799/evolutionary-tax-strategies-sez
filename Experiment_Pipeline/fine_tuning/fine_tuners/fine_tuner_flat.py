...
from Experiment_Pipeline.fine_tuning.wrappers.flat_wrapper import run_flat_w2
...
def parse_candidate_hp(params: Tuple[float, float]) -> Dict[str, Any]:
    """
    Parse a tuple of hyperparameters into a candidate dictionary.

    Parameters
    ----------
    params : tuple(float, float)
        Tuple containing (mutation_rate, parents_rate).
    ...
    """
    return {"mutation_rate": params[0], "parents_rate": params[1]}
...
def test_candidate(
    candidate_tuple: Tuple[float, float],
    ...
    ----------
    candidate_tuple : tuple(float, float)
        Candidate (mutation_rate, parents_rate).
    selected_envs : dict
    ...
    """
    candidate_hp = {"mutation_rate": candidate_tuple[0], "parents_rate": candidate_tuple[1]}
    print(f"\n[▶] Testing candidate: mutation_rate={candidate_hp['mutation_rate']:.3f}, "
...
def worker_test_candidate(args: Tuple[Tuple[float, float], Dict[str, Any], Dict[str, Any]]) -> Dict[str, Any]:
...
def run_fine_tune_flat(
    ...
    """
    Fine-tune GA hyperparameters (mutation_rate, parents_rate) via Latin Hypercube sampling.
    Logs results continuously so partial progress is not lost.
    ...
    # Scale from [0, 1] → [0.01, 0.9]
    candidate_grid = 0.0 + candidate_grid * (0.5 - 0.0)
    ...
    def log_result(candidate_result: Dict[str, Any]) -> None:
        mutation_rate, parents_rate = candidate_result["candidate"]
        ...
        print(
            f"[✔] Saved candidate (mutation_rate={mutation_rate:.3f}, "
            f"parents_rate={parents_rate:.3f}) → "
