...
# (Generic GA version — tuning mutpb and cxpb)
# =============================================================================
...
def parse_candidate_hp(params: Tuple[float, float]) -> Dict[str, Any]:
    """
    Parse a tuple of 2 hyperparameters into a candidate dictionary.
    ...
    """
    return {"mutpb": params[0], "cxpb": params[1]}
...
def test_candidate(
    ...
    print(
        f"\n[▶] Testing candidate: "
        f"mutpb={candidate_hp['mutpb']:.3f}, cxpb={candidate_hp['cxpb']:.3f}"
    )
...
def run_fine_tune_generic_ga(
    ...
    """
    Fine-tune Generic GA hyperparameters (mutpb, cxpb) via Latin Hypercube sampling.
    ...
    def log_result(candidate_result: Dict[str, Any]) -> None:
        mutpb, cxpb = candidate_result["candidate"]
        ...
        print(
            f"[✔] Saved candidate (mutpb={mutpb:.3f}, cxpb={cxpb:.3f}) → "
